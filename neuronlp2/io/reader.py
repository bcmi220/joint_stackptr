...
class CoNLLUReader(object):
    ...
    def getNext(self, normalize_digits=True, symbolic_root=False, symbolic_end=False):
        line = self.__source_file.readline()
        # skip multiple blank lines and comment lines .
        ...
        while len(line.strip()) > 0 and line.strip()[0]!='#':
            ...
            if line_data[0].isdigit(): # to skip the
        ...
        for tokens in lines:
            ...
            word = utils.DIGIT_RE.sub(b"0", tokens[1]) if normalize_digits else tokens[1] # FORM
            pos = tokens[3] # 4 is XPOSTAG while 3 is UPOSTAG, the XPOSTAG is unavailable sometimes, I use UPOSTAG here
            head = int(tokens[6]) # HEAD
            type = tokens[7] # DEPREL
