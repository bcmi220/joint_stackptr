...
if __name__ == '__main__':
    ...
    start, end = get_input_params(sys.argv[1:])
    ...
    for i in range(len(model_data)):
        ...
        input_dir = os.path.join('data/ud-treebanks-v2.2', 'UD_'+item['name'])
        models_dir = os.path.join('models/joint', item['name'])
        if not os.path.exists(models_dir):
            os.makedirs(models_dir)
        
        # train this
        if item.get('dev') is not None:

            train_file = os.path.join(input_dir,item.get('train')+'-ud-train.conllu')
            dev_file = os.path.join(input_dir,item.get('dev')+'-ud-dev.conllu')

            command = '''python examples/JointStackPointerParser_CoNLLU.py --mode FastLSTM --num_epochs 100 --batch_size 32 --decoder_input_size 256 --hidden_size 512 --encoder_layers 3 --decoder_layers 1 \
                    ...
                    --grandPar --sibling \
                    --word_embedding sskip --word_path "data/sskip/'''+item['dev']+'''.skip.forms.50.vectors.gz" --char_embedding random \
                    --punctuation '.' '``' "''" ':' ',' \
                    --train "'''+train_file+'''" \
                    --dev "'''+dev_file+'''" \
                    --test "'''+dev_file+'''" \
                    --model_path "'''+models_dir+'''/" --model_name "network.pt" '''
